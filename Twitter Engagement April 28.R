...
Coulter <- rio::import("Desktop/Data/Coulter.csv")
Coulter <- janitor::clean_names(Coulter)
#Date processing
Coulter <- Coulter %>%
  mutate(created_at = ymd_hms(created_at))
#
AOC <- rio::import("Desktop/Data/AOC.csv")
...
ggplot(AOCfavorites1)+
  ...
  scale_y_continuous(limits = c(0,800000), breaks = seq(0,800000,50000)) +
...
ggplot(Coulterfavorites1)+
  ...
  scale_fill_distiller(palette ="RdBu", direction = +1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(limits = c(0,150000), breaks = seq(0,150000,50000)) +
...
AOC_Coulter_Favorites <- rbind(AOCfavorites1, Coulterfavorites1)

ggplot(AOC_Coulter_Favorites)+
  aes(x = ymd, y = favorite_count, fill = sentiment,) +
  geom_col(show.legend = T) +
  scale_fill_distiller(palette ="RdBu", direction = +1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(limits = c(0,800000), breaks = seq(0,800000,50000)) +
  labs(title = "Ann Coulter And Acasio Cortes Favorites and Sentiment", 
       subtitle = "Ann Coulter And Acasio Cortes Twitter Feed",
       caption = "Source: Twitter 2019",
       x="Tweets",
       y="Most Popular Tweets") 
